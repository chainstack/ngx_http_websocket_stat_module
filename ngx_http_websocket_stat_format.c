               ...
               void *data)
{
    ...
    if (!template_cmpl) {
        ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0, "TEMPLATE STRUCT IS NULL");
    }
    if (!template_cmpl->compiled_template_str) {
        ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0, "COMPILED TEMPLATE IS NULL");
    }
    ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0, template_cmpl->compiled_template_str);
